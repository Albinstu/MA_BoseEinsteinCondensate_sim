...
delta = 0.5 # change to larger maybe 1 or 2
...
T = 1 # end time
N = 100 # number of time steps from 0th
dt = T / N # time step
dt_inv = N / T
...
psi1_new_old_N = fem.Function(V)
psi2_new_old_N = fem.Function(V)
psi1_new_old_N.name = "Psi 1 next t-step last newton iter"
psi2_new_old_N.name = "Psi 2 next t-step last newton iter"


# create function to store sol and update
ps1_new = fem.Function(V)
ps2_new = fem.Function(V)


# create newton step parameter
d_1 = ufl.TrialFunction(V)
d_2 = ufl.TrialFunction(V)
d_1_old_N = fem.Function(V) # old newton iteration
d_2_old_N = fem.Function(V) 



#####
# b1 = fem.Function(fem.functionspace(ext_domain_mesh, ("Lagrange", 1, 
#                                                       (ext_domain_mesh.geometry.dim,))))
# b1.interpolate(lambda x : np.vstack([np.full(x.shape[0], 1.0+0.0j), 
#                                     np.full(x.shape[0], -1.0j), np.full(x.shape[0], 0.0j)]))

b1_vec = ufl.as_tensor([1.0 + 0.0j, -1.0j, 0.0j]) # WORKS AND IS CLEAN
b2_vec = ufl.as_tensor([1.0 + 0.0j, 1.0j, 0.0j]) # WORKS AND IS CLEAN

# b1 = ufl.as_vector([1.0 + 0.0j, -1.0j, 0.0j])  # DOES NOT WORK
#####
...
v_conj = ufl.conj(v) # conjugate


# create non_linear func and derivatives
...
F_psi_1_deriv = lambda psi1, psi2 : g * ( ufl.conj(psi1) + ufl.conj(psi2) )
# F_psi_2_deriv = lambda psi1, psi2 : g3 * ufl.conj(psi1) + g2 * ufl.conj(psi2)
F_psi_1_conj_deriv = lambda psi1, psi2 : g * ( psi1 + psi2 )
# F_psi_2_conj_deriv = lambda psi1, psi2 : g3 * psi1 + g2 * psi2



# CONSTRUCT BILINEAR AND LINEAR FORMS

# initiate the newton stepper and first time step
# psi1_old = ...
# psi1_next_last_newton = 
...
################################################################################################
################################################################################################
# THE ISSUE HERE IS THAT I AM INCONSEQUENTIAL WITH HOW I REPRESENT THE TESTFUNCTION
# IN PLAIN MULTIPLIC THEN NEEED ufl.conj(v), BUT ufl.inner() AUTOMATICALLY 
# APPLIS THE CONJUGATION
################################################################################################
################################################################################################
################################################################################################
################################################################################################

F_new_old_N = F_nonlin_func(psi1_new_old_N, psi2_new_old_N)
F_old = F_nonlin_func(psi1_old, psi2_old)
F_1_deriv = F_psi_1_deriv(psi1_new_old_N, psi2_new_old_N)
F_2_deriv = F_1_deriv
F_1_conj_deriv = F_psi_1_conj_deriv(psi1_new_old_N, psi2_new_old_N)
F_2_conj_deriv = F_1_conj_deriv


# =============================================================================
# a1 = j_im * dt_inv * d_1 * v * ufl.dx -\
#     0.25 * ufl.inner( ufl.grad(d_1), ufl.grad(v) ) * ufl.dx -\
#         0.5 * pot_V * d_1 * v * ufl.dx +\
#             j_im * 0.5 * CAP_W * d_1 * v * ufl.dx -\
#                0.5 * F_new_old_N * d_1 * v * ufl.dx -\
#                    0.5 * psi1_new_old_N * F_1_deriv * d_1 * v * ufl.dx
# 
# 
# # constant in newton iterations
# L1_const = j_im * dt_inv * psi1_old * v * ufl.dx +\
#     0.25 * ufl.inner( ufl.grad(psi1_old), ufl.grad(v) ) * ufl.dx -\
#         0.5 * j_im * u * ufl.dot(b1, ufl.grad(psi2_old)) * v * ufl.dx +\
#             0.5 * pot_V * psi1_old * v * ufl.dx -\
#                 0.5 * j_im * CAP_W * psi1_old * v * ufl.dx +\
#                     0.5 * F_old * psi1_old * v * ufl.dx
# 
# # varying for newton iter
# L1 = - j_im * dt_inv * psi1_new_old_N * v * ufl.dx +\
#     0.25 * ufl.inner( ufl.grad(psi1_new_old_N), ufl.grad(v)) * ufl.dx -\
#         0.5 * j_im * u * ufl.dot(b1 , ufl.grad(psi2_new_old_N)) * v * ufl.dx
# L1 += 0.5 * pot_V * psi1_new_old_N * v * ufl.dx -\
#     0.5 * j_im * CAP_W * psi1_new_old_N * v * ufl.dx +\
#         0.5 * F_new_old_N * psi1_new_old_N * v * ufl.dx
# L1 += 0.5 * ( F_2_deriv * d_2_old_N + F_1_conj_deriv * ufl.conj(d_1_old_N) +\
#              F_2_conj_deriv * ufl.conj(d_2_old_N) ) * psi1_new_old_N * v * ufl.dx
# L1 += - 0.5 * j_im * u * ufl.dot(b1, ufl.grad(d_2_old_N)) * v * ufl.dx
# L1 += L1_const
# 
# 
# 
...
a1_const = (j_im * 2 * dt_inv - pot_V + j_im * CAP_W) * d_1 * v_conj * ufl.dx \
    - 0.5 * ufl.inner( ufl.grad(d_1), ufl.grad(v) ) * ufl.dx
# a1_const += - 0.5 * ufl.inner( ufl.grad(d_1), ufl.grad(v) ) * ufl.dx

a1 = - ( F_new_old_N + F_1_deriv * psi1_new_old_N ) * d_1 * v_conj * ufl.dx

a1 += a1_const



L1_const = (j_im * 2 * dt_inv + pot_V - j_im * CAP_W + F_old) * psi1_old \
    * v_conj * ufl.dx \
        +0.5 * ufl.inner(ufl.grad(psi1_old), ufl.grad(v)) * ufl.dx \
            - j_im * u * ufl.dot(b1_vec, ufl.grad(psi2_old)) * v_conj * ufl.dx

L1 = (- j_im * 2 * dt_inv + pot_V - j_im * CAP_W + F_new_old_N \
      + F_2_deriv * d_2_old_N + F_1_conj_deriv * ufl.conj(d_1_old_N) \
          + F_2_conj_deriv * ufl.conj(d_2_old_N)) \
    * psi1_new_old_N * v_conj * ufl.dx
L1 += 0.5 * ufl.inner(ufl.grad(psi1_new_old_N), ufl.grad(v)) * ufl.dx
L1 += - j_im * u * ufl.dot(b1_vec, ufl.grad(psi2_new_old_N)) * v_conj * ufl.dx
L1 += - j_im * u * ufl.dot(b1_vec, ufl.grad(d_2_old_N)) * v_conj * ufl.dx
